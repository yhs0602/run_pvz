...
### 2026-02-26 11:45 KST XML 루프 추가 가속 실험(결과: 제한적)
- 코드 변경 (`main.cpp`)
  - XML append run fast-path 추가:
    - `0x5a1f9a`(state=5 경로), `0x5a207a`(state=4 경로)에서 한 글자씩 `0x441d20`를 호출하던 루프를
      작은 배치(최대 256자)로 묶어서 append 후 분기 복귀.
    - 계측 카운터 `xmlrun` 추가.
  - 문자열 비교 fast-path 추가:
    - `0x4565a0` 비교 함수를 host-side 비교로 단축(`cmp4565a0` 카운터 추가).
    - `0x456610`/`0x441a60` 호출 왕복을 일부 감소시키는 목적.
- 검증 로그
  - `artifacts/probes/probe_xmlrun_counter_20260226_113813.log`
    - `xmlrun`이 실제 증가(`~98`)하는 것은 확인.
    - 다만 전체 hot 체인 대비 비중이 낮아 체감 개선은 제한적.
  - `artifacts/probes/probe_cmp4565a0_20260226_114152.log`
    - `cmp4565a0` 카운터가 지속 증가(`~25k` 이상)하며 fast-path 적용은 확인.
    - 그러나 block-hot 상위는 여전히 `0x456610/0x5a1640/0x5bd830/0x61be1b/0x5a1f60/0x5a217d` 군집.
    - 샘플 구간 내 `IDirectDrawSurface7::Lock/Unlock` 미관측.
- 결론
  - 두 패치 모두 “적용은 됨”이 확인되었지만, 렌더 루프 진입을 막는 주 병목을 해소하기에는 부족.
  - 다음 단계는 `0x456610` 상위 caller 체인(특히 parser state 전이와 비교/append 묶음)을 더 큰 단위로 단축하거나,
    파서 완료 조건을 직접 계측해 진입 게이트를 고정하는 방향이 필요.
