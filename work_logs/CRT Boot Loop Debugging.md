...
이 프로젝트는 Plants Vs Zombies 구버전 (32비트 윈도우 게임)을 Apple M1 에서 돌려보기 위한 에뮬레이터 프로젝트야.

# [Project Context] x86 Win32 to ARM64 macOS 에뮬레이터 (LLM-Assisted JIT Compiler)

## 1. Project Overview
* **목표:** 20년 전 x86 32비트 Windows 프로그램을 최신 Apple Silicon(ARM64) macOS에서 실행하는 하이브리드 에뮬레이터 개발.
* **핵심 아이디어:** 기존 인터프리터 방식의 느린 속도를 극복하기 위해, 실행 중 발견된 x86 Basic Block을 LLM을 활용해 ARM64 네이티브 코드로 JIT/AOT 컴파일하여 캐싱하는 "Cloud-based Profile-Guided JIT" 아키텍처.

## 2. Tech Stack & Framework
* **Core Engine:** `Unicorn Engine` (x86 Fallback 실행 및 Context 관리)
* **Analysis:** `Capstone` (x86 Disassembly 및 레지스터 속성 분석)
* **JIT Assembler:** `Keystone` (LLM이 생성한 ARM64 텍스트를 기계어로 변환)
* **OS/API Layer:** Wine의 HLE(High-Level Emulation) 구조 차용 (Fake DLL & Thunking)
* **Graphics:** DXVK + MoltenVK 조합을 통한 DirectX -> Metal 변환 (후순위)

## 3. Core Technical Strategies (The Secret Sauce)

### A. Execution Pipeline (Hybrid JIT)
1. `UC_HOOK_BLOCK`을 통해 Unicorn이 새로운 Basic Block에 진입할 때마다 가로챔.
2. 실행 횟수(Hotspot)를 프로파일링하여 임계치를 넘으면 백그라운드 스레드에서 LLM 비동기 호출.
3. 번역이 완료된 블록은 메모리에 실행 권한(mmap)으로 적재하고, 다음 번 해당 주소 진입 시 Unicorn을 멈추고(`uc_emu_stop`) 네이티브 ARM64 코드로 Context Switch 수행.

### B. Live Variable Analysis (LVA) 기반 LLM Prompting
* **문제:** LLM이 x86 전체 문맥을 알 수 없고, Context Switch 오버헤드가 크며, Lazy Flag 상태가 깨짐.
* **해결 (Static Analysis):** Capstone의 `cs_regs_access()`를 이용해 Basic Block 단위로 역방향 스캔(Backward Data-flow Analysis) 수행.
* **LVA 산출물:**
  * `Live-In`: 블록 시작 전 Unicorn Context 구조체에서 반드시 읽어와야 할 레지스터.
  * `Live-Out`: 블록 종료 후 Unicorn Context에 다시 써야 할 레지스터.
  * `Dead`: 내부 임시 계산용 (동기화 무시).
  * `Flags (EFLAGS)`: 독립된 변수로 취급하여 분기에 필요한 플래그만 명시적 계산(Materialize).
* **LLM 컨텍스트 주입:** 추출된 Live-In/Out 명세와 x86 어셈블리만 LLM 프롬프트에 주입하여 군더더기 없는 ARM 코드를 생성하도록 강제함.
* **초기 타협안:** 블록 단위 분석의 한계를 고려하여, 초기 버전은 "현재 블록에서 수정된 모든 레지스터는 무조건 Live-Out"으로 간주하는 보수적(Conservative) 접근 채택. 추후 Trace-based (블록 병합) LVA로 고도화.

## 4. AI Agent SOP (Standard Operating Procedure)
*이 프로젝트를 수행하는 AI(너)는 아래의 'Senior Dev Workflow'를 엄격히 따른다.*

1. **Systematic Approach:** 즉흥적인 원숭이 패치(Monkey patching)를 금지한다. 코드를 작성하기 전에 반드시 설계와 대안을 브레인스토밍(Brainstorming)한다.
2. **Action Plan:** 작업을 2~5분 단위의 아주 작은 마일스톤으로 쪼개고(Planning), 어떤 파일을 수정할지, 어떻게 검증할지 먼저 제시한다.
3. **Verification First:** 구현(Implementation) 후에는 반드시 테스트 코드를 작성하거나 실행 결과를 통해 작동을 증명(Verification)한 뒤에 다음 단계로 넘어간다.
4. **Root-cause Tracing:** 버그 발생 시, 표면적 에러 메시지에 의존하지 않고 콜 스택을 거꾸로 추적하여 근본 원인을 찾는다.
5. **Context Management:** LVA 로직과 Context Switch 구조체(`struct CPUContext`) 설계 시 메모리 안전성과 레지스터 매핑 ABI 규칙을 최우선으로 고려한다.

## 5. Next Immediate Step
에뮬레이터의 가장 뼈대가 되는 **"Python + Unicorn + Capstone을 이용한 Basic Block 후킹 및 LVA(Live-In/Out) 추출 프로토타입"** 코드 작성을 시작할 것.
...
근데 아직 LLM 연동 부분은 안 해도 될듯? 이건 일단 사람이 해봐도 되니까 (감을 잡아야 llm한테 잘 시키지)
...
PE 파서 라이브러리를 추가하고, pvz/main.exe를 실행하는 걸 목표로 할게. bass.dll도 있네
...
### User Input
 ...
 Windows SEH 및 PEB/TEB를 위한 FS 레지스터와 GDT(Global Descriptor Table) 세팅 쪽으로 넘어가서 애플리케이션 초기화 과정을 더 깊게 돌려줘
...
앞으로 진행할 때 업무일지를 계속 작성하도록 해. 그거를 agents.md같은데 써놓으면 안 잊어버릴듯
1. 리팩토링 먼저 하고
2. 시작하고
3. 도 진행하자.
...
지시하신 대로 프로젝트 최상단 디렉토리에 전용 업무일지 폴더(`work_logs/`)를 만들어 구성했습니다. 📁

- 방금 전까지 사용했던 기본 명세서([agents.md](cci:7://file:///Users/yanghyeonseo/Developer/pvz/agents.md:0:0-0:0))는 이 폴더 안으로 이동하여 **[work_logs/master_plan.md](cci:7://file:///Users/yanghyeonseo/Developer/pvz/work_logs/master_plan.md:0:0-0:0)** (전체 로드맵)로 정리해 두었습니다.
- 방금 마친 아주 큰 작업(OOP 모듈화 및 C++ 커스텀 엔진 완벽 이식) 내용의 성과를 정리하여, 오늘 자 업무일지인 **[work_logs/2026-02-21_refactoring.md](cci:7://file:///Users/yanghyeonseo/Developer/pvz/work_logs/2026-02-21_refactoring.md:0:0-0:0)** 에 첫 번째 기록으로 남겼습니다.

앞으로는 이 폴더 안에서 차근차근 단계별(Step-by-Step) 진행 상황과 설계 결정 사항을 기록하며 전진하겠습니다!

이제 파일 정리까지 완벽하게 끝났으니, 다음 단계인 **Phase 3: JIT(Just-In-Time) 컴파일 파이프라인 (루프 핫스팟 식별 및 LLM ARM64 번역 연동)** 에 대한 고민과 설계를 시작해 볼까요? 궁금하신 점이나 특별히 먼저 고려해야 할 사항이 있다면 지시해 주세요!
...
*User accepted the command `gss                     ✔  pvz   7   21:47:33  `*

*User accepted the command `gss                     ✔  pvz   7   21:47:34  `*

*User accepted the command `ga .                    ✔  pvz   7   21:47:37  `*

*User accepted the command `gc                         ✔  pvz   7   21:47:41  `*

*User accepted the command `gp                   ✔  8s   pvz   7   21:47:50  `*
...
레지스터를 이용한 점프 (indirect jump) 등은 지금 어떻게 처리되고 있어?

### Planner Response

현재 저희가 구축한 JIT 에뮬레이터에서 **레지스터를 이용한 점프 (Indirect Jump, 예: `jmp eax`나 `call ebx`)**는 어떻게 처리되는지, 그리고 앞으로 구축할 **ARM64 번역(JIT) 단계에서는 어떻게 처리할 계획**인지 두 가지 관점으로 나누어 명확히 설명해 드리겠습니다.

### 1. 현재 구조 (C++ & Unicorn 엔진 상태)
지금 단계에서는 C++에서 `Unicorn Engine`이 코어 실행을 전담하고 있습니다. 
Unicorn은 완전한 x86 에뮬레이터이기 때문에 **레지스터(Indirect) 점프를 스스로 완벽하게 처리**합니다.
- `jmp eax` 명령어를 만나면, Unicorn이 내부적으로 레지스터 값을 읽어 해당 EIP(명령어 포인터)로 점프합니다.
- 점프 직후 새로운 주소(Target)에 도달하면, 코어에 달아둔 우리의 훅 `UC_HOOK_BLOCK`이 자동으로 발동합니다.
- 즉, 정적 분석 도구와 달리, 런타임에 직접 블록 단위로 추적 중이기 때문에 간접 점프 대상이 동적으로 결정되더라도 **우리의 JIT 핫스팟 프로파일러는 그 대상 주소 값을 완벽하고 정확하게 캡처**하고 있습니다.

### 2. 향후 계획 (LLM을 이용한 ARM64 번역 실행 시의 문제와 처리 방안)
이제 추출된 이 블록 페이로드를 LLM을 거쳐 **ARM64** 네이티브 머신 코드로 번역하고, 메모리에 할당해 직접 실행(Execute)하게 되는 Phase 3의 후반부를 계획해야 합니다. 이때 간접 점프가 굉장히 까다로운 요소가 됩니다.

번역된 ARM64 코드 끝에 x86의 `jmp eax`에 대응하는 레지스터 점프가 오면, 해당 점프는 ARM64 생태계의 메모리 주소로 뛰어야 하지만 `eax`에는 원본 x86 메모리 주소(예: `0x631000`)가 들어있기 때문입니다.

이를 해결하기 위해 **JIT Dispatcher (혹은 Trampoline)** 아키텍처를 도입해야 합니다.

**[ 처리 흐름 구상 ]**
1. **번역 단계 (LLM)**: LLM이 x86의 `jmp eax`를 번역할 때, 바로 간접 점프를 뛰는게 아니라, 목표 주소(`eax`)를 C++ 엔진(또는 문맥 레지스터)에 저장한 뒤 원래의 C++ 즉, `JIT Dispatcher`로 **제어권을 반환(Return)** 하도록 지시합니다. (이때의 Live-Out에는 `eax`가 포함됩니다)
2. **JIT Dispatcher의 역할**: 제어권을 넘겨받은 디스패처는 반환된 목표 주소(`eax`에 든 실제 x86 주소)를 **번역된 ARM64 블록 캐시 테이블(Map)** 에서 검색합니다.
   - **(A) 이미 번역된 블록이 있을 경우**: 즉시 해당 ARM64 함수 포인터로 빠르게 직행합니다. (Block Chaining)
   - **(B) 아직 번역되지 않은 경우**: Unicorn 에뮬레이터의 `EIP`를 조작한 뒤 Unicorn에게 하드웨어 제어권을 넘겨, 번역될 때까지 Fallback 에뮬레이션을 돌립니다.

따라서 지금 구조의 **동적 핫스팟 프로파일링(Dynamic Profiling)** 은 가장 확실하게 간접 점프의 목적지들을 프로파일링하고 핫스팟으로 만들고 있으며, 추후 JIT 매핑 테이블(Dispatcher)을 거치게끔 LLM 번역 결과물에 제약을 거는 방식으로 Indirect Jump를 완벽히 통제하게 됩니다! 

이 JIT Dispatcher 설계를 다음 [master_plan.md](cci:7://file:///Users/yanghyeonseo/Developer/pvz/work_logs/master_plan.md:0:0-0:0) 및 [task.md](cci:7://file:///Users/yanghyeonseo/.gemini/antigravity/brain/f2d8326e-54f2-4b63-b993-85fc4d2d82bd/task.md:0:0-0:0) 에 기록하고 LLM 프롬프트 디자인으로 넘어갈까요? 궁금하신 점이 있다면 편하게 말씀해주세요.
...
*User accepted the command `codex                                                               ✔  pvz   7   21:47:52  `*

*User accepted the command `source /Users/yanghyeonseo/Developer/pvz/.venv/bin/activate           ✔  7   00:25:58  `*

*User accepted the command `ls                                                                  ✔  pvz   7   00:26:00  `*

*User accepted the command `codex --help                                                        ✔  pvz   7   00:26:00  `*

### User Input

- MacOS에서 JIT이 권한이슈 W^X 이런거 괜찮은가?
일단 llm 번역기 파이썬 봇을 구현하되 llm은 codex-cli의 exec 기능을 써서 하는거로 해줘
...
*User accepted the command `ga .                                                                ✔  pvz   7   00:52:02  `*

*User accepted the command `gc                                                                     ✔  pvz   7   00:52:04  `*

*User accepted the command `gp                                                              ✔  6s   pvz   7   00:52:10  `*

*User accepted the command `ls                                                                        ✔  pvz   7   00:52:12  `*

*User accepted the command `open .                                                                    ✔  pvz   7   00:52:13  `*

### User Input

- codex LLM 호출은 느리고 비싸고, 우리는 계속 테스트하고 돌릴거니까 캐싱같은걸 하면 좋겠어
- API 콜을 모킹하는 일이 남았네. 설계를 해야 하고
- 우리의 작업을 더 자세히 설명하는 문서를 한글로 남기면 좋겠어.
...
모킹을 하되 가능하면 실제 동작을 잘 하도록 하는게 좋겠어. 이것도 jit과 비슷한 패턴으로 진행하게 될 것 같네. 구현 안 된 API를 llm으로 구현하고 캐싱하고 등등...

추가 우려점은 API들은 간혹 stateful 할수있다는점?
...
한글 문서도 추가로 상세히 업데이트해줘. 그리고 남은 할 일이 뭐야?
...
그러면 이제 pvz.exe 실행이 가능해? 방법도 readme 등의 문서로 써줘

*Edited relevant file*

### User Input

DLL 로딩 이런거랑 사용자 입력 루프 이런거 필요하텐데 그런것도 모킹이 잘 되려나 

*Edited relevant file*

### User Input

정말 놀랍고도 광기 어린(물론 최고의 칭찬입니다!) 프로젝트입니다. **LLM을 JIT 컴파일과 동적 API 모킹에 실시간으로 개입시키는 아키텍처**는 매우 실험적이고 혁신적입니다. 콘솔에서 AI가 미지의 API를 만나 스스로 코드를 짜며 앞으로 나아가는 모습을 지켜보는 것은 엄청난 희열일 것입니다.

작성해주신 비유처럼, 지금은 **"최첨단 엔진을 만들었지만, 운전대(입력)와 앞유리창(디스플레이)이 연결되지 않은 상태"**입니다. 게임이 실제로 구동되려면 순수 에뮬레이션을 넘어 **Host OS(macOS)와의 연결(Bridging)**, 즉 **HLE(High-Level Emulation) 레이어 구축**으로 넘어가야 합니다.

다음 단계로 나아가기 위한 구체적인 로드맵과 구현 전략을 4가지 페이즈로 나누어 제안해 드립니다.

---

### Phase 1: Host 브릿지 구축 및 윈도우/입력 시스템 연동 (SDL2 도입)

가장 먼저 해야 할 일은 Mac 환경에서 창을 띄우고 입력을 받을 수 있는 **기반 프레임워크를 메인 C++ 엔진에 연동**하는 것입니다. 크로스 플랫폼 지원이 강력한 **SDL2**나 **GLFW**를 추천합니다.

1. **메인 엔진에 SDL2 초기화:** C++ 에뮬레이터가 켜질 때 백그라운드에서 SDL2 창을 하나 생성해 둡니다. (초기엔 빈 검은 화면)
2. **메시지 루프(Event Loop) 하드코딩 후킹:**
* `api_compiler.py`가 `GetMessageA`, `PeekMessageA` 같은 핵심 입력 API를 무작정 모킹하게 두지 마세요.
* 이러한 **Core API들은 메인 C++ 엔진에 직접 하드코딩하여 Intercept** 해야 합니다.
* 게임이 `GetMessageA`를 호출하면 -> 엔진이 실행을 가로채서 `SDL_PollEvent()`를 호출 -> Mac의 마우스/키보드 움직임을 Windows의 `MSG` 구조체(WM_MOUSEMOVE, WM_KEYDOWN 등) 포맷으로 변환 -> 에뮬레이터 안의 가상 메모리 주소에 써주고 반환합니다.



### Phase 2: 그래픽 렌더링 파이프라인 구축 (DirectDraw / DirectX)

PvZ는 구형 게임이므로 주로 **DirectDraw (2D)** 또는 구형 **DirectX (3D)**를 사용합니다. 화면을 그리는 것은 LLM이 짜주는 코드로 해결할 수 없으며, 물리적인 그래픽 번역 레이어가 필요합니다.

1. **DirectDraw API 가로채기:** `DirectDrawCreate`, `CreateSurface` 같은 API 호출을 가로챕니다.
2. **가상 VRAM 버퍼 제공:** 게임이 표면(Surface)을 만들어 그림을 그리려고 할 때, 엔진의 가상 메모리 내에 빈 버퍼 공간을 할당해 주고 그 주소를 게임에 반환합니다. 게임은 윈도우인 줄 알고 그 메모리 공간에 픽셀 데이터를 씁니다.
3. **Host 화면에 그리기 (Blitting):** 메인 C++ 엔진의 루프에서 해당 가상 버퍼의 변경 사항을 감지하고, 이를 `SDL_Texture`로 변환하여 Mac 화면(SDL 윈도우)에 뿌려줍니다 (RenderCopy).

### Phase 3: LLM의 역할 고도화 (Mocker에서 Router로)

지금까지의 LLM은 "에러를 안 내고 무조건 성공을 반환하는 가짜 코드"를 짰다면, 이제부터는 **"엔진이 제공하는 브릿지 함수(Host API)를 호출하는 코드"**를 짜도록 프롬프트를 업그레이드해야 합니다.

1. **엔진 내부 SDK 노출:** C++ 엔진 내부에 `Host_CreateWindow`, `Host_DrawRect`, `Host_PlaySound` 같은 헬퍼 함수들을 만들어 둡니다.
2. **API 데몬 프롬프트 엔지니어링:**
* `api_compiler.py`에 다음 지침을 추가합니다: *"네가 `CreateWindowExA`라는 Windows API를 마주치면, 그냥 0을 리턴하지 말고 메인 엔진의 `Host_CreateWindow`를 호출해서 그 반환값을 넘겨주는 C++ 코드를 작성해."*
* 이렇게 하면 LLM은 수많은 Windows UI/그래픽 API들을 내가 만든 Mac용 브릿지 함수들과 연결해 주는 **지능형 라우터(Router)** 역할을 하게 됩니다.



### Phase 4: 파일 시스템 및 오디오 (VFS & Audio Wrapper)

화면이 나오기 전이나 나올 즈음에 게임이 리소스(`.pak` 파일, 이미지, 효과음)를 읽으려다 죽을 확률이 높습니다.

1. **파일 입출력 (`CreateFileA`, `ReadFile`):** Windows 경로(`C:\games\pvz\main.pak`)를 Mac의 실제 경로(`/Users/name/pvz/main.pak`)로 매핑해주는 **가상 파일 시스템(VFS)** 함수를 C++ 엔진에 구현하고 매핑해야 합니다.
2. **오디오 (`DirectSound` 또는 `WaveOut`):** 그래픽과 마찬가지로 게임이 보내는 PCM 오디오 버퍼를 가로채서 SDL_Audio를 통해 Mac 스피커로 출력해야 합니다.

---

### 💡 다음 행동 추천 (Next Step)

가장 먼저 성취감을 느낄 수 있고 디버깅에 큰 도움이 되는 첫 단추는 **Phase 1(창 띄우기)과 Phase 4(파일 시스템 연결)**입니다. 게임이 `main.pak` 같은 리소스 파일을 성공적으로 읽고 빈 창(Window)을 띄우는 것까지만 성공해도 엄청난 진전입니다.


1. **메인 C++ 엔진에 SDL2를 연동해서 빈 창을 띄우고 `GetMessage` 루프를 브릿징하는 방법**
2. **게임이 Mac의 파일 시스템에 접근할 수 있도록 `CreateFile`, `ReadFile` API를 C++ 레벨에서 가로채는 가상 파일 시스템(VFS) 구축 방법**
3. **LLM 프롬프트를 수정하여 가짜 모킹 대신, 엔진의 Host 함수를 호출하도록 유도하는 프롬프트 엔지니어링 방법**
...
다음 단계는 “LLM이 모킹으로 넘길 수 없는 실제 I/O(창·입력·그래픽·사운드) 를 호스트(macOS)에서 구현해, 게스트(Win32) 호출을 결정론적으로 받아주는 서브시스템을 붙이는 일입니다. PvZ는 실행 환경에 따라 DirectDraw 초기화 실패가 직접적으로 보고되는 편이라, 최소한의 user32 메시지 루프 + DirectDraw(2D) 프레젠테이션이 먼저 필요합니다.  ￼

아래는 “첫 화면(타이틀/메뉴)까지”를 목표로 한, 구현 난이도 대비 진전이 큰 순서의 로드맵입니다.

⸻

1) Win32 “창 + 메시지 루프”를 진짜로 만든다 (user32 최소 집합)

목표
	•	PvZ가 CreateWindowEx → ShowWindow → GetMessage/PeekMessage 루프로 들어가도 멈추지 않고,
	•	macOS 이벤트(키/마우스/종료)가 WM_ 메시지*로 들어가게 만들기.

구현 포인트(최소 API)
	•	윈도우/클래스: RegisterClassExA/W, CreateWindowExA/W, DestroyWindow, DefWindowProc, GetClientRect, AdjustWindowRect
	•	메시지: GetMessage, PeekMessage, TranslateMessage, DispatchMessage, PostMessage, SendMessage
	•	타이머/시간: SetTimer/KillTimer, GetTickCount, QueryPerformanceCounter/Frequency
	•	포커스/커서(필요 시): GetCursorPos, SetCursorPos, ShowCursor

백엔드 선택(추천)
	•	SDL2로 창 생성 + 이벤트 폴링 → Win32 MSG 큐로 변환 (가장 구현량 대비 효과 큼)
	•	또는 Cocoa 직접(더 네이티브하지만 구현량 증가)

여기서 중요한 건 “핸들(HWND/HINSTANCE)과 WNDPROC 디스패치”를 에뮬레이터 내부 오브젝트 테이블로 관리하고, 게스트에는 32-bit 값(opaque handle)만 노출하는 구조입니다.

⸻

2) 입력 브릿지: macOS 이벤트 → Win32 MSG 변환을 고정 구현한다

목표
	•	LLM 더미가 아닌, 실제 사용자 입력이 게임 로직으로 들어감

변환 테이블(최소)
	•	마우스: WM_MOUSEMOVE, WM_LBUTTONDOWN/UP, WM_RBUTTONDOWN/UP
	•	키보드: WM_KEYDOWN/UP, WM_CHAR (키 매핑/스캔코드 최소)
	•	종료: WM_QUIT

구현 팁
	•	처음엔 IME/조합키/가속키/RAWINPUT 다 빼고, PvZ가 필요한 범위만.
	•	좌표계 변환(윈도우 좌상단 기준)과 클릭 캡처 정도만 추가.

⸻

3) 그래픽: DirectDraw(2D) “표시”를 최소 기능으로 뚫는다

PvZ가 DirectDraw 계열을 건드리는 정황은 흔하고(“Failed to initialize DirectDraw”류 에러), 결국 화면이 뜨려면 ddraw.dll 호출을 성공 반환이 아니라 “프레임버퍼를 갱신해서 실제로 보여주기”까지 가야 합니다.  ￼

목표(타이틀 화면까지의 최소 기능)
	•	DirectDrawCreate/Ex로 디바이스 생성
	•	CreateSurface로 primary/backbuffer/offscreen surface 생성
	•	Lock/Unlock로 CPU가 픽셀 버퍼를 채움
	•	Blt/BltFast/Flip 중 최소 1개로 primary에 합성
	•	호스트 창에 present

권장 구현 전략(현실적인 MVP)
	•	DirectDraw 인터페이스를 “완전 구현”하려고 하지 말고,
	•	IDirectDraw(7)/IDirectDrawSurface(7) vtable 중 PvZ가 실제 호출하는 서브셋만 먼저 구현
	•	Surface는 호스트 메모리 버퍼(RGBA) 로 시작(소프트웨어 블릿)
	•	present는 SDL2 텍스처 업로드(또는 Metal)로 한 장면씩 표시
	•	포맷은 보통 RGB565/555/ARGB8888 정도부터 시작

참고할 만한 구현 소스/구조
	•	Wine/ReactOS 쪽 DirectDraw 구현 구조는 인터페이스/레퍼런싱/서피스 관리 방식 이해에 도움 됩니다.  ￼
	•	“옛 DirectDraw 게임 호환”을 다루는 wrapper 프로젝트들은 어떤 API/구조체 함정이 많은지 힌트를 줍니다(예: surface desc 크기, DX 버전별 차이 등).  ￼

핵심은: LoadLibrary/GetProcAddress는 이미 우회가 잘 되므로, ddraw.dll만큼은 “LLM 모킹”이 아니라 엔진에 내장된 네이티브 모듈(호스트 플러그인) 로 제공하는 게 맞습니다.

⸻

4) 사운드: winmm(또는 DirectSound) 최소 재생만 구현

목표
	•	BGM/효과음이 나면서 “게임이 진짜로 돌아간다” 단계 진입

우선순위
	•	1순위: winmm.dll의 waveOut* 최소 집합 → SDL2 Audio로 매핑
	•	2순위: dsound.dll (필요할 때)

⸻

5) 나머지 “필수 OS 유사 기능” 체크리스트

게임이 화면 이후에 막히는 지점은 대개 아래 중 하나입니다.
	•	스레딩/동기화: CreateThread, Tls*, CRITICAL_SECTION, WaitForSingleObject, 이벤트/뮤텍스
	•	파일/경로: GetModuleFileName, GetTempPath, SHGetFolderPath(shell32)
	•	레지스트리: RegOpenKeyEx, RegQueryValueEx(advapi32)
	•	폰트/GDI 일부: 텍스트 렌더링을 DirectDraw 위에 얹는 경우(최소는 “더미 성공”으로도 넘어갈 때가 있으나 결국 UI가 깨질 수 있음)

⸻

6) “다음 구현을 무엇부터?”에 대한 추천 결론

PvZ 기준으로 가장 빠르게 “가시적 진전(첫 프레임)”을 얻는 순서는:
	1.	SDL2 기반 user32 메시지 루프 + HWND/WNDPROC 디스패치
	2.	입력 이벤트 변환(마우스/키보드)
	3.	DirectDraw surface + Lock/Unlock + (Blt 또는 Flip) + present
	4.	그 다음에 winmm waveOut

이 4개가 붙으면 “콘솔 로그 감상” 단계에서 “실제 창이 뜨고, 타이틀이 그려지고, 클릭이 먹는” 단계로 넘어갑니다.

⸻

구현 작업을 더 빠르게 만드는 실전 팁 3가지
	1.	API 트레이스 기반으로 ‘실호출 목록’을 먼저 고정

	•	PvZ가 부르는 user32/ddraw/winmm 함수 이름/호출 순서/구조체 크기만 뽑아, 그 목록을 “MVP 스펙”으로 삼으면 범위가 급격히 줄어듭니다.

	2.	LLM은 “코드 생성”이 아니라 “스텁 스펙 작성 + 테스트 케이스 생성”으로 역할 변경

	•	I/O 계열은 결정론/디버깅이 중요해서, 여기부터는 LLM이 실시간으로 바꿔 끼우는 방식이 오히려 불안정해질 수 있습니다.

	3.	첫 목표는 DirectX 전체가 아니라 ‘DirectDraw 2D blit 파이프’

	•	PvZ는 2D 성격이 강하므로, 3D/셰이더/복잡한 COM 전체보다 “서피스-블릿-프리젠트”가 성패를 가릅니다.
...
현황 보고해줘. boot_trace.txt 가 2.3G로 거대하니 조심
