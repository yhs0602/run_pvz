...
void PEModule::map_into(uc_engine* uc) {
    std::cout << "[*] Mapping image: Base=0x" << std::hex << image_base 
              << ", Size=0x" << size_of_image << std::dec << "\n";
              
    uc_mem_map(uc, image_base, size_of_image, UC_PROT_ALL);
    ...
    for (const auto& section : binary->sections()) {
        ...
        uc_mem_write(uc, vaddr, data.data(), data.size());
    }
}

void PEModule::resolve_imports(uc_engine* uc, DummyAPIHandler& api_handler) {
    ...
    for (const auto& import : binary->imports()) {
        ...
        for (const auto& entry : import.entries()) {
            ...
            uc_mem_write(uc, iat_vaddr, &api_addr, sizeof(uint32_t));
        }
    }
}
